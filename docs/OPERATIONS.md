...
| 日付 | 内容 |
|---|---|
| 2026-04-07 | v2.0.1 対応: CIワークフロー (`.github/workflows/test.yml`) のバージョンチェックに post-merge 競合回避ロジックを追加（PR の HEAD がすでに main にマージ済みの場合はスキップ） |
| 2026-04-08 | v2.1.0 対応: 既読通知・完了タスクの30日自動削除ポリシーを導入。「13. トラブルシューティング」に「データが勝手に消えた」項目追加予定。v2.0.2 scope（起動時パフォーマンス改善）も統合。**クラウド同期ペイロードを gzip 圧縮（format 2）に変更**。初回同期後にユーザーの Firestore ドキュメントが自動的に圧縮形式へ移行する。format 1（旧プレーン JSON）も引き続き読める |
| 2026-04-07 | v2.0.0 対応: アプリ名「ユメハシ」への正式移行に伴うメジャーバージョンアップ手順を反映 |
